...
abstract class SmiteAPIHelper {
	...
	// Smite API System, Default is PC
...
//		}
	...
	/*
	 * Set the system for the API to use
	 * $system: string containing the system type, must correspond to either PC or XBOX
	 * Returns a boolean indicating if XBOX or PC is set
	 */
	public static function setSystem($system) {
		
		if ($system == "XBOX") {
			SmiteAPIHelper::$_system = "XBOX";
			return true;
		} else if ($system == "PC") {
			SmiteAPIHelper::$_system = "";
			return false;
		}
	}
	...
	private static function executeRequest($url) {
		...
		//Change API URL depending on the system set
		if (SmiteAPIHelper::$_system == "XBOX") {
			//Xbox API
			$ch = curl_init("http://api.xbox.smitegame.com/smiteapi.svc/".$url);
		}
